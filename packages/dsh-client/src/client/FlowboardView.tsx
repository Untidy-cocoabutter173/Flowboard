...
function MeetingDetail({ snapshot, meeting, command, navigate }: { snapshot: FlowboardSnapshot; meeting: MeetingView; command(value: ClientCommand): Promise<unknown>; navigate(route: FlowboardRoute): void }) {
  ...
  return <section><PageHeader title={meeting.title} meta={`${meeting.status} · ${formatDate(meeting.startedAt ?? meeting.createdAt)}`} actions={<><Button variant="outline" size="sm" onClick={() => setEditing('transcript')}>编辑转录稿</Button><Button variant="primary" size="sm" onClick={() => setEditing('summary')}>编辑总结</Button></>} /><div className={css.meetingDetail}><section><h3>转录稿</h3>{utterances.map(item => <p key={item.id}><time>#{item.sequence}</time>{item.text}</p>)}{utterances.length === 0 && meeting.transcript !== '' && <p>{meeting.transcript}</p>}{utterances.length === 0 && meeting.transcript === '' && <Empty title="等待会议转录" />}</section><aside><h3>核心总结</h3><p>{meeting.summary || '会议结束后由 AI 整理'}</p><h3>决议</h3>{meeting.decisions.map(item => <p key={item}>{item}</p>)}<h3>风险</h3>{meeting.risks.map(item => <p key={item}>{item}</p>)}<h3>会议资料</h3>{snapshot.library.filter(item => documents.has(item.id)).map(item => <button className={css.documentLink} type="button" key={item.id} onClick={() => navigate({ area: 'library', libraryId: item.id })}>{item.title}</button>)}<h3>AI 操作</h3>{actions.map(item => <p key={item.id}>{item.summary}</p>)}</aside></div>{editing !== null && <MarkdownEditor open title={editing === 'summary' ? `${meeting.title} · 会议总结` : `${meeting.title} · 转录稿`} fileName={`${meeting.title.replace(/\s+/g, '-').toLowerCase()}-${editing}.md`} value={editing === 'summary' ? meeting.summary : meeting.transcript} onClose={() => setEditing(null)} onSave={value => command({ type: 'meeting.update', expectedVersion: meeting.version, payload: { id: meeting.id, [editing]: value } })} />}</section>
}
