...
export function LibraryView({
  ...
  const items = snapshot.library.filter(
    (item) => projectId === null || linked.has(item.id),
  );
  ...
  return (
    <section>
      ...
      <div>
        {items.map((item) => (
          <article className={css.libraryRow} key={item.id}>
      ...
      {(dialog?.type === "create" || dialog?.type === "edit") && (
        <EntityModal
          open
          title={editor === undefined ? "新建资料" : "编辑资料"}
          submitLabel="保存"
          ...
          onSubmit={async (data) => {
            ...
            const existingProjects =
              editor === undefined
                ? []
                : snapshot.links.projectLibrary
                    .filter((link) => link.libraryItemId === editor.id)
            ...
            const projectIds =
              projectId === null
                ...
                : editor === undefined
                  ? [projectId]
                  : existingProjects;
            if (projectIds.length === 0) throw new Error("至少关联一个项目");
          ...
          <div className={css.formGrid}>
            ...
            <Field label="类型">
              <SelectControl
                name="type"
                ariaLabel="资料类型"
                defaultValue={editor?.type ?? "doc"}
                disabled={editor !== undefined}
                options={[
                  { value: "doc", label: "Markdown 文档" },
                  { value: "link", label: "链接" },
                ]}
              />
            </Field>
      ...
      {dialog?.type === "delete" && (
        <ConfirmDialog
          open
          title="删除资料"
          detail={`确定删除“${dialog.item.title}”吗？`}
          onClose={() => setDialog(null)}
          onConfirm={async () =>
            command({
              type: "library.delete",
              expectedVersion: dialog.item.version,
              payload: { id: dialog.item.id },
            })
          }
        />
      )}
