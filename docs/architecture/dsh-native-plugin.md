...
> 🆕 本文定义 Flowboard 作为官方 DSH 插件的当前实现与开源发布基线。代码、CI 和发布流程必须同时满足这些约束。

## 1. 产品与宿主边界 🆕

Flowboard 是 DSH 的任务管理插件，不是 DSH 的替代宿主，也不是让 DSH 反向接入的独立应用。DSH 负责人和 Agent 的完整办公入口、Session、profile、插件安装、Host/Client 注入、Web 启动和销毁；Flowboard 在这个生命周期内，把目标、会议、Agent 执行和产出组织为共享任务状态。
