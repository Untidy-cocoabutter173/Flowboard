...
```mermaid
flowchart LR
  DSH[DSH web profile] -->|加载唯一插件包| PB[@flowboard/dsh]
  PB --> SC[DSH Web Client]
  PB --> SS[FlowboardService]
  PB --> SA[Agent Tools]
  SC -->|Typert Remote| SS
  SA --> HC[Host HTTP Client]
  SS --> HC
  SS -->|拥有生命周期| API
  SS -->|拥有生命周期| WORKER
...
- 独立 server 与动态诊断 Host 需要显式 `FLOWBOARD_TOKEN`；公开插件的嵌入模式不向浏览器暴露 Token。
